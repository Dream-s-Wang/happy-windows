...
def add_text(text, image_path, save_path_name, text_RGBA=(255, 255, 255, 50), text_xy=(1.0, 1.0), text_font='C:\Windows\Fonts\simhei.ttf', text_font_size=100):
    ...
    # 文字位置
    text_xy = ((layer.size[0]-text_size_x)*text_xy[0],
               (layer.size[1]-text_size_y)*text_xy[1])
...
add_text(time.strftime('%Y-%m-%d %A', time.localtime(time.time())),
         sys.argv[1], tmp, text_xy=(0.98, 0.98), text_font_size=60)
setWallPaper(tmp)
